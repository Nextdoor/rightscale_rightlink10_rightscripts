...
def get_ebs_volumes(awskey, awssecret, ebs_vol_list, volcount, volsize,
    ...
    ec2 = boto.connect_ec2( aws_access_key_id = awskey, aws_secret_access_key = awssecret)
    ec2 = boto.ec2.connect_to_region(region, aws_access_key_id = awskey, aws_secret_access_key = awssecret)
...
if options.action == "instance":
    ...
    if not options.awskey or not options.awssecret: parser.error("both awskey and awssecret must be supplied")
    ...
    if not options.awskey or not options.awssecret: parser.error("both awskey and awssecret must be supplied")
    if not options.volids: parser.error("--volids/-i must be supplied with a comma-separated list of volume ids")
    volid_array = options.volids.split(',')
    vols = find_ebs_volumes(options.awskey, options.awssecret, DEFAULT_EBS_DISK_NAMES, volid_array)
